<?php
   ...
   if(isset($_POST["Submit"])){
	...
	if($_POST["ability"] == "လုပ္သက္(နွစ္)"){
		$abilityError = '*လုပ္သက္ရိုက္ထည့္ရန္ လိုအပ္ပါသည္။ မရွိပါက မရွိဘူးဟုေရးနိုင္သည္';
   }else{
		$ability = Test_User_Input($_POST["ability"]);
   }
  if(($_POST["name"] != "အမည္")&&($_POST["email"] != "အီးေမးလိပ္စာ")){
	if(preg_match("/^[A-Za-z. ]*$/",$Name)==true && preg_match("/[a-zA-Z0-9._-]{3,}@[a-zA-Z0-9._-]{3,}[.]{1}[a-zA-Z0-9._-]{2,}/",$Email)&&!empty($city)&&!empty($ability)&& !empty($Radio) && !empty($Check) && preg_match("/[A-Z0-9a-z.]{8,}/",$Password) && preg_match("/[A-Z0-9a-z.]{8,}/",$Password1) && ($Password == $Password1)&& preg_match("/[0-9]{5,}/",$PhoneNumber)){
		function redirect_to($NewLocation){
			header("Location:".$NewLocation);
			exit;
		}
		redirect_to("sentCode.php?Email=$Email&Name=$Name&Phone=$PhoneNumber&City=$city&Password=$Password&Trader=$Check&Gender=$Radio&Ability=$ability");
		} 
    }
}
...
<html>
<head>
<title>G&C Supervisor Sign Up Form</title>
...
<div class="log">
	<div class="content2">
		...
		<form action="Supervisor Sign Up.php" method="post">
            ...
            <input type="text" name="ability" placeholder="လုပ္သက္(နွစ္)" Value = "လုပ္သက္(နွစ္)" onfocus="this.value = '';" onblur="if (this.value == '') {this.value = 'လုပ္သက္(နွစ္)';}">
			<div class = "Error"><?php echo $abilityError;?></div>
		...
		<h2><a href="Supervisor Login Form.php">ရွိျပီးသား အေကာင့္ကို ဝင္ရန္</a></h2>
	</div>
</div>
<div class="footer">
	<p>© 2018 G&C Supervisor Sign Up Form</p>
</div>
